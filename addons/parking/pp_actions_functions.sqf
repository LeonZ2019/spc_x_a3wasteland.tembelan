...
pp_setup_terminal = {
  ...
  if (isServer) then
  {
    ...
    def(_laptop);
    _laptop = createSimpleObject ["Land_Laptop_03_olive_F", getPosASL _terminal]; //createVehicle ["Land_Laptop_unfolded_F", getPosASL _terminal, [], 0, "NONE"];
    _laptop allowDamage false;
    _laptop attachTo [_terminal, [0,-0.1,0.55]];
    // leon we can set laptop texture here
    _laptop setObjectTextureGlobal [0, "addons\parking\icons\display.paa"];
...
pp_create_terminal = {
 //Land_Laptop_unfolded_F
  ...
  _terminal = createVehicle ["Land_PortableDesk_01_olive_F", ASLtoATL _pos, [], 0, "NONE"];
...
pp_is_player_near = {
  private["_objects"];
  _objects = nearestObjects [player, ["Land_PortableDesk_01_olive_F"], 2];
...
} forEach ((allMissionObjects "Land_PortableDesk_01_olive_F") select {_x getVariable ["is_parking",false]});

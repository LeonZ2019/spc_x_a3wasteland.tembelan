...
A3W_bleedingTime = 300;             // Time in seconds for which to allow revive after a critical injury (minimum 10 seconds) - to disable revive, go in addons\far_revive\FAR_defines.sqf
...
A3W_artilleryRadius = 30;          // Strike radius in meters within which the shells will randomly hit
A3W_artilleryCooldown = 10;     // Cooldown in seconds between strike uses per individual player - resets on server restart
A3W_artilleryCrateOdds = 0.4;     // Odds of finding one Artillery Strike in a mission crate (0.0 to 1.0)
...
A3W_spawnBeaconCooldown = 60;    // Number of seconds to wait between each use of a specific spawn beacon (0 = disabled)
...
A3W_poiObjLockDistance = 25;      // Prevent players from locking objects within this distance from points of interest (stores & mission spawns)
...
A3W_atmTransferFee = 2;            // Fee in percent charged to players for money transfers to other players (0 to 50)
...
A3W_mineLifetime = 0;           // Maximum lifetime in hours for saved mines across server restarts (0 = no time limit)
...
A3W_privateParkingLimit = 12;       // Maximum amount of vehicles allowed in private parking (0 = no limit)
...
A3W_vehicleQuantity = 135;         // Approximate number of land vehicles to be spawned in towns
...
A3W_missionFarAiDrawLines = 1;     // Draw small red lines on the map from mission markers to individual units & vehicles which are further away than 90m from the objective (0 = no, 1 = yes)
...
A3W_militaryMissionTimeout = 30*60; // Time in seconds that a Military Mission will run for, unless completed
